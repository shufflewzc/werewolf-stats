# werewolf-stats

一个面向狼人杀竞技赛事的数据仓库，用来维护战队、队员、照片路径、对战记录，以及类似 NBA 队员页的统计结果。

## 当前支持

- 战队资料：战队 ID、名称、队标路径、队员名单
- 队员资料：队员 ID、所属战队、照片路径、别名、状态
- 标准化赛事记录：赛季、阶段、轮次、局次、阵营胜负、个人角色表现
- 自动统计：胜率、站边率、得分率、场均得分、存活率
- 自动生成排行榜：队员榜和战队汇总

## 目录结构

```text
.
|-- assets/
|   |-- players/
|   `-- teams/
|-- data/
|   |-- werewolf_stats.db
|-- reports/
|   |-- dashboard.html
|   |-- player_leaderboard.json
|   |-- player_leaderboard.md
|   |-- team_summary.json
|   `-- team_summary.md
|-- schemas/
|   |-- match.schema.json
|   |-- player.schema.json
|   `-- team.schema.json
`-- scripts/
    |-- web/
    |   |-- features/
    |   |   |-- guilds.py
    |   |   |-- profile.py
    |   |   `-- team_center.py
    |   `-- README.md
    |-- generate_stats.py
    |-- migrate_json_to_sqlite.py
    |-- sqlite_store.py
    |-- validate_data.py
    `-- web_app.py
```

## 存储方式

当前系统已经升级为 `SQLite` 存储，主数据库文件为：

- `data/werewolf_stats.db`

网站、校验脚本和报表脚本现在都直接读写 `SQLite`，不再在运行时自动依赖 `JSON` 文件。

Web 层当前已经开始按业务模块拆分：

- `scripts/web_app.py` 负责兼容入口、共享工具和路由分发
- `scripts/web/features/` 负责按功能拆分页面与处理器
- 目前已经独立出门派、个人中心、战队操作三个模块，后续可以继续按赛事、比赛、权限等域拆分

如果你手头还有旧版 `JSON` 数据，需要显式执行一次迁移：

```bash
python3 scripts/migrate_json_to_sqlite.py
```

迁移完成后，`SQLite` 会成为唯一主存储。

## 数据设计

### `teams` 表

保存战队主数据。

- `team_id`: 战队唯一 ID
- `name`: 战队名称
- `short_name`: 战队简称
- `logo`: 战队队标或图片路径
- `active`: 是否仍在使用
- `founded_on`: 建队日期
- `members`: 当前队员 ID 列表
- `notes`: 备注

### `players` 表

保存队员资料。

- `player_id`: 队员唯一 ID
- `display_name`: 展示名
- `team_id`: 当前所属战队
- `photo`: 队员照片路径
- `aliases`: 别名列表
- `active`: 是否活跃
- `joined_on`: 加入数据库日期
- `notes`: 备注

### `matches` 与 `match_players` 表

保存标准化赛事对局记录。

- `match_id`: 对局唯一 ID
- `season`: 赛季名
- `stage`: 比赛阶段，如 `regular_season`
- `round`: 轮次
- `game_no`: 该轮第几局
- `played_on`: 日期
- `table_label`: 台次或房间号
- `format`: 板型，如 `classic-12`
- `duration_minutes`: 对局时长
- `winning_camp`: 胜利阵营，支持 `villagers`、`werewolves`、`third_party`、`draw`
- `players`: 每位上场队员的对局记录
- `notes`: 备注

单个队员对局记录字段：

- `player_id`: 队员 ID
- `team_id`: 本局代表的战队
- `seat`: 座位号
- `role`: 角色名
- `camp`: 所属阵营
- `survived`: 是否存活到结束
- `result`: 个人胜负
- `points_earned`: 本局得分
- `points_available`: 本局满分
- `stance_pick`: 本局站边结果，支持 `villagers`、`werewolves`、`third_party`、`none`
- `stance_correct`: 站边是否正确
- `notes`: 备注

## 统计口径

- 胜率 `win_rate` = 胜场 / 出场
- 站边率 `stance_rate` = 正确站边场次 / 有明确站边场次
- 得分率 `score_rate` = 累计得分 / 累计可得分
- 场均得分 `average_points` = 累计得分 / 出场
- 存活率 `survival_rate` = 存活场次 / 出场

战队汇总按“战队队员总出场表现”统计，不是假设狼人杀一定存在严格的战队对战胜负表。

## 使用方式

先校验数据：

```bash
python3 scripts/validate_data.py
```

再生成统计报表：

```bash
python3 scripts/generate_stats.py
```

脚本会输出到：

- `reports/dashboard.html`
- `reports/player_leaderboard.json`
- `reports/player_leaderboard.md`
- `reports/team_summary.json`
- `reports/team_summary.md`

生成后可以直接双击 `reports/dashboard.html` 在浏览器中打开。
如果想用本地地址测试，也可以运行：

```bash
python3 -m http.server
```

然后访问 `http://localhost:8000/reports/dashboard.html`

## 登录版网站

如果你想使用“可登录、可编辑”的网站版本，运行：

```bash
python3 scripts/web_app.py
```

然后打开：

```text
http://localhost:8000
```

当前默认账号：

- 用户名：`admin`
- 密码：`admin123`

登录后你可以：

- 查看首页和比赛列表
- 先进入比赛页面，再选择该比赛下的战队
- 进入每个比赛里的战队页面
- 查看该战队按赛事、按赛季整理的全部比赛表格
- 点击“编辑比赛”进入比赛编辑页并保存到 `data/werewolf_stats.db`
- 进入“战队操作”页面，创建新战队、申请加入已有战队、发起转会申请
- 由目标战队队长在“战队操作”页面审核加入申请和转会申请
- 在没有历史比赛记录且没有待处理申请时，直接退出当前战队
- 使用 `admin` 进入“账号管理”页面，新增或删除登录账号
- 在首页和队员页按单独赛事切换查看统计
- 战队战绩和排名只在单个比赛里统计，不做跨比赛综合榜
- 在队员页同时查看综合统计和分赛事统计

当前也支持公开注册：

- 打开 `/register`
- 填写用户名、显示名称、密码
- 完成一个简单加法验证码
- 注册后即可登录，并创建战队或申请加入战队

当前主存储已经切换为 `SQLite`：

- 数据库文件：`data/werewolf_stats.db`
- 用户、战队、队员、比赛、战队申请都保存在同一个 SQLite 数据库中
- 比赛记录支持单独的“赛事名称”字段，例如“京城大师赛广州公开赛”或“LAL广州公开赛”
- 如需把旧 JSON 数据迁移进数据库，可运行 `python3 scripts/migrate_json_to_sqlite.py`

页面中的当前时间按中国时间展示。

## 备注

- `assets/players/` 和 `assets/teams/` 用来存放照片和队标文件
- 当前仓库默认可初始化为空库，适合直接上线后录入正式数据
- 后续可以继续扩展角色专属数据，例如预言家命中率、猎人开枪命中率、女巫救毒收益等

...
- 数据校验与统计报表生成
- 本地网站形式的公开展示与后台录入
- `PostgreSQL` 作为生产主库，`SQLite` 用于本地开发和迁移验证
- `ios/` 下的原生 SwiftUI iPhone 浏览客户端，可复用同一套公开 JSON 接口
...
4. **持久化**：生产业务数据写入 PostgreSQL；`data/werewolf_stats.db` 仅用于本地开发、迁移对照和离线验证。
...
生产环境使用 PostgreSQL。开发环境未配置 `DATABASE_URL` 时，可以使用 SQLite：

- `data/werewolf_stats.db`

网站、校验脚本和报表脚本通过统一 Repository 访问数据库，不再依赖运行时自动同步 JSON 文件。
...
- `GET /api/competitions`
  ...
  小程序赛事 Tab 使用 `grouped=1` 查询参数获取全部城市分组和兼容的扁平赛事列表
...
- 在单一比赛口径下查看战队战绩和排名
- 在队员页同时查看综合统计和分赛事统计
- 在“赛事管理 → 编辑赛季档期 → 赛段设置”中为当前赛事、当前赛季自定义赛段显示名称；留空恢复默认名称，不会改变历史比赛的底层赛段代码

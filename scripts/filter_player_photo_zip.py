...
def fetch_json(url: str, params: dict[str, object]) -> dict[str, object]:
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "werewolf-stats-player-photo-matcher/1.0",
        },
    )
    try:
...
def load_manual_selections(selection_path: str) -> dict[str, str]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("人工选择文件格式不正确，应为 player_id 到图片相对路径的映射。")
    return {
        str(player_id).strip(): str(source_file).strip()
        for player_id, source_file in payload.items()
...
def apply_manual_selections(
    ...
    if not selections:
        return matches, 0
    folder = folder.resolve()
    indexes_by_player_id: dict[str, list[int]] = defaultdict(list)
    for index, item in enumerate(matches):
        if item.status == "duplicate" and item.player:
            indexes_by_player_id[item.player.player_id].append(index)

    resolved = list(matches)
    confirmed_count = 0
    for player_id, indexes in indexes_by_player_id.items():
        selected_source = selections.get(player_id, "")
        selected_indexes = [
            index
            for index in indexes
            if str(matches[index].path.relative_to(folder)) == selected_source
        ]
        ...
        for index in indexes:
            ...
            if index == selected_index:
                resolved[index] = PhotoMatch(
                    ...
                    player=item.player,
                    status="included",
                    method=f"{item.method}，App 内人工确认",
                )
            else:
                resolved[index] = PhotoMatch(
                    path=item.path,
                    ...
                    status="rejected",
                    method=item.method,
                    reason="App 内人工确认时未选用",
                )
        confirmed_count += 1
    return resolved, confirmed_count
...
def write_zip(output_zip: Path, matches: list[PhotoMatch]) -> int:
...
<main>
  ...
  <div class="summary">
    ...
    <div class="metric"><b>{status_counts['included']}</b>自动收录</div>
  ...
  <p class="hint">只有绿色“已收录”项目会进入 ZIP。多图候选可以直接在 Mac App 内选择，确认后会重新生成最终 ZIP。</p>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "从混合图片文件夹中筛选当前赛季现有选手头像，并生成后台可直接上传的 ZIP。"
        )
    )
    parser.add_argument(
        "--folder",
...
def main() -> int:
    ...
    try:
        ...
        matches, confirmed_count = apply_manual_selections(folder, matches, selections)
        rows = report_rows(folder, matches, players)
        written_count = write_zip(output_zip, matches)
        ...
        write_html_report(preview_path, folder, matches, players, scope_label)
    ...
    print(f"自动收录：{written_count} 张")
    print(f"人工确认：{confirmed_count} 组")
